    ...
    .then(() => {
      ...
      Object.entries(obj_config).forEach((x) => {
        ...
        if (key === "contract_address") {
          ...
          window.localStorage.setItem("contract_address", contract_token);
        } else if (key === "contract_logo") {
          contract_logo = entry["_value"];
          window.localStorage.setItem("contract_logo", contract_logo);
        } else if (key == "chainID") {
          chain_ID = entry["_value"];
          window.localStorage.setItem("chain_ID", chain_ID);
        }
      });
    })
    .catch(() => {
      return "Error: Failed to get remote configuration data";
    });

  contract_token = window.localStorage.getItem("contract_address");
  contract_logo = window.localStorage.getItem("contract_logo");
      ...
      <Router>
        <Header style={styles.header}>
          ...
          <div style={styles.headerRight}>
            <TokenPrice
              address="0x31471e0791fcdbe82fbf4c44943255e923f1b794" //{JSON.stringify(token_contract)} 0x1f9840a85d5af5bf1d1762f925bdaddc4201f984 token uniswap
              chain="bsc"
              image={contract_logo}
              size="40px"
            />
            ...
            <TokenBalance
              token_address={JSON.stringify(contract_token)}
              chain_ID={chain_ID}
            />
            <Account />
          </div>
        ...
        <div style={styles.content}>
          <Switch>
            ...
            <Route path="/1inch">
              <Tabs defaultActiveKey="1" style={{ alignItems: "center" }}>
                <Tabs.TabPane tab={<span>Binance Smart Chain</span>} key="2">
                  <DEX chain="bsc" customTokens={customTokens} />
                </Tabs.TabPane>
              </Tabs>
            </Route>
            ...
            <Route path="/NFT">
              <Tabs defaultActiveKey="1" style={{ alignItems: "center" }}>
                <Tabs.TabPane tab={<span>NFT Market</span>} key="1">
                  <NFT />
                </Tabs.TabPane>
                <Tabs.TabPane tab={<span>Mint your NFT</span>} key="2">
                  <NFT />
                </Tabs.TabPane>
              </Tabs>
            </Route>
