...
import app_data from "./main_data";
...
const App = ({ isServerInfo }) => {
  ...
  if (app_data) {
    console.log("Paso por 0");
    remoteConfig = getRemoteConfig(app_data);
    remoteConfig.settings.minimumFetchIntervalMillis = 10000;
    remoteConfig.defaultConfig = {
      contract_token: "0x0000000000000000000000000000000000000000",
      contract_logo:
        "https://cdn.iconscout.com/icon/free/png-256/cryptocurrency-1889496-1597556.png",
    };
  } else {
    console.log("No se ha podido conectar a la base de datos.");
  }

  async function obt_remoteConfig() {
    console.log("Paso por 1");
    await fetchAndActivate(remoteConfig);
    return getAll(remoteConfig);
    //contract_logo = JSON.stringify(obj_config.contract_logo._value);
    //console.log(contract_logo);
  }
  obj_config = obt_remoteConfig();
  ...
  if (obj_config) {
    ...
    /*contract_token = JSON.stringify(obj_config.contract_address._value);
    console.log("Do aqui " + JSON.stringify(obj_config));*/
  }
  ...
  //var params_config = JSON.stringify(obj_contract());
  //console.log("Params: " + JSON.stringify(obj_config.contract_address._value));
  //contract_token = JSON.stringify(params_config.contract_address._value);
      ...
      <Router>
        <Header style={styles.header}>
          ...
          <div style={styles.headerRight}>
            <Chains />
            <TokenPrice
              address="0x1f9840a85d5af5bf1d1762f925bdaddc4201f984" //{JSON.stringify(token_contract)} 0x1f9840a85d5af5bf1d1762f925bdaddc4201f984 token uniswap
              chain="bsc"
              image={JSON.stringify(contract_logo)}
              size="40px"
            />
            <TokenBalance token_address={contract_token} balance_default="0" />
            <Account />
          </div>
        </Header>
